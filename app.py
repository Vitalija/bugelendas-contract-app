...
def contract():
    if request.method == "POST":
        ...
        telefono_numeris = request.form["telefonas"]
        el_pastas = request.form["elpastas"]
        stovekla = request.form["stovykla"]

        data = f"{vardas}_{pavarde}_{stovekla}"
        pdf = FPDF()
        pdf.add_page()
        pdf.set_font("Arial", size=12)
        pdf.cell(200, 10, txt="STOVYKLOS SUTARTIS", ln=True, align="C")
        pdf.ln(10)
        pdf.multi_cell(0, 10, txt=f"Vardas: {vardas}\nPavardė: {pavarde}\nTelefonas: {telefono_numeris}\nEl. paštas: {el_pastas}\nStovykla: {stovekla}\nPasirašymo data: {datetime.datetime.now().date()}")
        ...
        dropbox_path = f"/sutartys/{data}.pdf"
        dbx.files_upload(pdf_output.read(), dropbox_path, mute=True)

        return "Sutartis pateikta. Ačiū!"
